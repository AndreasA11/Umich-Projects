...
void Building::spawnPerson(Person newPerson){
    int personCurrentFloor = newPerson.getCurrentFloor();
    floors[personCurrentFloor].addPerson(newPerson,
                                         newPerson.getTargetFloor()
                                         - newPerson.getCurrentFloor());
}
...
void Building::update(Move move){
    if (move.isPickupMove() == true) {
        ...
        move.copyListOfPeopleToPickup(arr1);
        floors[floor].removePeople(arr1, move.getNumPeopleToPickup());
...
int Building::tick(Move move){
    ...
    for (int j = 0; j < NUM_FLOORS; j++) {
        total = total + floors[j].tick(time);
    }
